...
async function saveVideoWithMetadata(
  ...
  try {
    ...
    const filename = `seedance-${safeConcept}-${timestamp}.mp4`
    ...
    try {
      ...
      if (success) {
        console.log(`✅ Seedance video saved to database: ${metadataObject.id}`);
      } else {
        console.warn(`⚠️ Failed to save Seedance video to database: ${metadataObject.id}`);
      }
    } catch (error) {
      console.error('Error saving Seedance video to database:', error);
    }
    
    console.log(`Seedance video saved: ${videoPath}`)
    console.log(`Seedance metadata saved: ${metadataPath}`)
    
    return path.join('videos', 'clips', filename)
    
  } catch (error) {
    console.error('Error saving Seedance video with metadata:', error)
    throw error
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!reference_image_urls || !Array.isArray(reference_image_urls) || reference_image_urls.length === 0) {
      return NextResponse.json({ error: 'Reference image URLs are required (1-4 images)' }, { status: 400 })
    }

    if (reference_image_urls.length > 4) {
    ...
    console.log('Generating video with Bytedance Seedance model:', { 
      concept: concept || 'Seedance Reference-to-Video',
      aspect_ratio: finalAspectRatio,
      duration,
      resolution,
      reference_images: reference_image_urls.length
    })
    ...
    const result = await fal.subscribe('fal-ai/bytedance/seedance/v1/lite/reference-to-video', {
      ...
      onQueueUpdate: (update) => {
        if (update.status === 'IN_PROGRESS') {
          console.log('Seedance video generation in progress...')
        }
      },
    }) as FalSeedanceResponse
    ...
    if (save_to_disk && result.video?.url) {
      ...
      const metadata = {
        ...
        api_response: {
          ...
          model_used: 'fal-ai/bytedance/seedance/v1/lite/reference-to-video'
        },
        user_agent: request.headers.get('user-agent'),
        ip_address: request.headers.get('x-forwarded-for') || request.headers.get('x-real-ip'),
        request_id: `seedance-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
      }

      localPath = await saveVideoWithMetadata(
        result.video.url,
        metadata,
        concept || 'Seedance Reference-to-Video',
        currentProjectId
      )
      ...
      videoMetadata = {
        id: metadata.request_id,
        title: concept || 'Seedance Reference-to-Video',
        description: prompt,
      ...
      try {
        console.log('🔄 Immediately syncing new Seedance video to database...')
        ...
        if (syncResponse.ok) {
          ...
          console.log('✅ Seedance video immediately synced to database:', syncResult.message)
          ...
          try {
            ...
            if (appended) {
              console.log(`🧩 Added new Seedance video ${newVideoId} to the end of the timeline`)
            } else {
              console.warn(`⚠️ Failed to add new Seedance video ${newVideoId} to the timeline`)
            }
          } catch (e) {
            console.warn('⚠️ Error appending new Seedance video to timeline:', e)
          }
        } else {
          console.warn('⚠️ Failed to immediately sync Seedance video to database')
        }
      } catch (error) {
        console.warn('⚠️ Error immediately syncing Seedance video to database:', error)
      }
    }
    ...
    return NextResponse.json({
      ...enhancedResult,
      message: save_to_disk ? 'Seedance video generated and saved successfully' : 'Seedance video generated successfully',
    ...
    console.error('Error generating Seedance video:', error)
    return NextResponse.json(
      { 
        error: 'Failed to generate Seedance video', 
