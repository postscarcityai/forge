...
async function saveVideoWithMetadata(
  ...
  try {
    ...
    const filename = `kling-elements-${safeConcept}-${timestamp}.mp4`
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!input_image_urls || !Array.isArray(input_image_urls) || input_image_urls.length === 0) {
      return NextResponse.json({ error: 'input_image_urls array is required and must contain at least 1 image URL' }, { status: 400 })
    }

    if (input_image_urls.length > 4) {
      return NextResponse.json({ error: 'Maximum 4 images are supported for Kling Elements model' }, { status: 400 })
    }
    ...
    const processedImageUrls = await Promise.all(
      input_image_urls.map(async (url: string) => {
    ...
    const input = {
      prompt,
      input_image_urls: processedImageUrls,
      duration,
      aspect_ratio: finalAspectRatio,
      negative_prompt
    }

    console.log('Generating video with Kling Elements model:', { 
      concept: concept || 'Kling Elements Video Generation',
      aspect_ratio: finalAspectRatio,
      duration,
      image_count: input_image_urls.length
    })
    ...
    const result = await fal.subscribe('fal-ai/kling-video/v1.6/standard/elements', {
      ...
      onQueueUpdate: (update) => {
        if (update.status === 'IN_PROGRESS') {
          console.log('Kling Elements video generation in progress...')
        }
      },
    }) as FalVideoResponse
    ...
    if (save_to_disk && result.video?.url) {
      ...
      const metadata = {
        ...
        api_response: {
          ...
          model_used: 'fal-ai/kling-video/v1.6/standard/elements'
        },
        user_agent: request.headers.get('user-agent'),
        ip_address: request.headers.get('x-forwarded-for') || request.headers.get('x-real-ip'),
        request_id: `kling-elements-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
      }

      localPath = await saveVideoWithMetadata(
        result.video.url,
        metadata,
        concept || 'Kling Elements Video Generation',
        currentProjectId
      )
      ...
      videoMetadata = {
        id: metadata.request_id,
        title: concept || 'Kling Elements Video Generation',
        description: prompt,
    ...
    return NextResponse.json({
      ...enhancedResult,
      message: save_to_disk ? 'Kling Elements video generated and saved successfully' : 'Kling Elements video generated successfully',
      ...
      input_images_count: input_image_urls.length,
      generation_data: {
        seed: result.seed,
        model_used: 'fal-ai/kling-video/v1.6/standard/elements',
        input_parameters: {
    ...
    console.error('Error generating Kling Elements video:', error)
    return NextResponse.json(
      { 
        error: 'Failed to generate Kling Elements video', 
