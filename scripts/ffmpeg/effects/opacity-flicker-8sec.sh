...
# Custom 8-second flicker effect script
...
show_usage() {
    ...
    echo ""
    echo "Example:"
    echo "  $0 images/change-the-text-2025-06-24T19-03-49-695Z-00.jpg"
    echo ""
    echo "Notes:"
    echo "  • Creates exactly 8 seconds of flicker effect"
...
echo "🎬 Creating 8-second flicker video from: $img"
echo ""
...
# Generate 192 frames (8 seconds at 24fps)
total_frames=192
echo "🎬 Generating $total_frames frames for 8-second video..."
...
for i in $(seq 1 $total_frames); do
    ...
    if [ $((i % 24)) -eq 0 ]; then
        seconds=$((i / 24))
        echo "   Progress: ${seconds}s / 8s complete"
    fi
done

echo "✅ Generated $total_frames frames for 8.0s video"

# Generate output filename with timestamp
TIMESTAMP=$(date +%Y-%m-%dT%H-%M-%S)
BASENAME=$(basename "$img" | sed 's/\.[^.]*$//')
OUTPUT_FILE="$OUTPUT_DIR/${BASENAME}_flicker_8sec_$TIMESTAMP.mp4"
...
if [ $? -eq 0 ]; then
    echo "✅ 8-second flicker effect video created successfully!"
    echo "📁 Output: $OUTPUT_FILE"
    
    # Get video info for metadata
    VIDEO_INFO=$(ffprobe -v quiet -print_format json -show_format -show_streams "$OUTPUT_FILE")
    DURATION=$(echo "$VIDEO_INFO" | jq -r '.format.duration // "8.0"')
    ...
    UNIQUE_ID="flicker-8sec-$(date +%s)000-custom"
    ...
    cat > "$METADATA_FILE" << EOF
{
  "id": "$UNIQUE_ID",
  "filename": "$FILENAME",
  "title": "8-Second Flicker Effect - PostScarcity.ai/Learn",
  "description": "8-second atmospheric flicker effect video with random opacity variations creating mesmerizing glow effect",
  ...
  "metadata": {
    ...
    "source": "flicker-8sec-custom"
  },
  "tags": ["flicker", "atmospheric", "opacity", "postscarcity", "8-second", "glow", "animation"]
}
EOF
