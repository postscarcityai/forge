...
/**
 * Download Project Images Script (CommonJS Version)
 * 
 * Downloads all non-hidden images from the current project to a local directory.
 * 
 * Usage:
 *   node scripts/download-project-images-simple.js [options]
 * 
 * Options:
...
/**
 * Get all non-hidden images for a project, ordered by timeline
 */
function getProjectImages(db, projectId) {
  try {
    const query = `
      ...
      ORDER BY 
        CASE 
          WHEN timeline_order IS NOT NULL THEN timeline_order 
          ELSE 999999 
        END ASC,
        created_at DESC
    `;
    ...
    const timelineImages = images.filter(img => img.timeline_order !== null);
    const nonTimelineImages = images.filter(img => img.timeline_order === null);
    
    console.log(`📊 Found ${images.length} non-hidden images in project "${projectId}"`);
    console.log(`   📋 ${timelineImages.length} images in timeline order`);
    console.log(`   📅 ${nonTimelineImages.length} images by creation date`);
...
/**
 * Download a single image file with timeline ordering
 */
function downloadImage(image, outputDir, index) {
  ...
  try {
    ...
    if (options.dryRun) {
      const timelineInfo = image.timeline_order !== null ? ` [Timeline: ${image.timeline_order}]` : ' [By date]';
      console.log(`[DRY RUN] Would copy: ${prefixedFilename}${timelineInfo} (${formatBytes(stats.size)})`);
    ...
    const timelineInfo = image.timeline_order !== null ? ` [Timeline: ${image.timeline_order}]` : ' [By date]';
    console.log(`✅ Downloaded: ${prefixedFilename}${timelineInfo} (${formatBytes(stats.size)})`);
...
function createMetadataFile(projectId, images, outputDir, results) {
  ...
  const metadata = {
    ...
    nonTimelineImages: nonTimelineImages.length,
    ...
    orderingInfo: {
      description: "Images are numbered based on their timeline order (001, 002, etc.). Images with timeline_order appear first, followed by images ordered by creation date.",
      timelineOrderedFirst: true
    },
